...
function defaultShowAlert(message, type = 'info', containerId = 'alerts-container') {
  ...
  alertDiv.role = 'alert';
  alertDiv.innerHTML = `
        <i class="bi bi-${normalizedType === 'success' ? 'check-circle-fill' : normalizedType === 'danger' ? 'exclamation-circle-fill' : 'info-circle-fill'} mr-2"></i>
    ...
    if (theme === 'modern') {
      return `
                <button type="button" class="modern-btn modern-btn-white modern-btn-sm js-oauth-link-btn" data-provider="${escapeHtml(provider)}">
                    <i class="bi bi-${icon} mr-1"></i> Link ${name} Account
                </button>
            `;
    }
                    ...
                    <div class="admin-panel-card-body p-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            ...
                            <span class="badge ${isPrimary ? 'bg-primary' : 'bg-success'}">
                                <i class="bi bi-${isPrimary ? 'star-fill' : 'check-circle'} mr-1"></i>
                                ${isPrimary ? 'Primary' : 'Connected'}
                            </span>
                        </div>
                        <p class="text-muted small mb-3">Linked on ${escapeHtml(linkedDate)}</p>
                        <div class="d-flex gap-2">
                            ${isPrimary ? '' : `<button type="button" class="modern-btn modern-btn-primary modern-btn-sm js-oauth-set-primary" data-provider="${escapeHtml(provider)}"><i class="bi bi-star mr-1"></i> Set Primary</button>`}
                            <button type="button" class="modern-btn modern-btn-danger modern-btn-sm js-oauth-unlink" data-provider="${escapeHtml(provider)}">
                                <i class="bi bi-unlink mr-1"></i> Unlink
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
                <div>
                    <div class="d-flex align-items-center">
                        <i class="bi bi-${icon} mr-2 fs-5"></i>
                        <div>
