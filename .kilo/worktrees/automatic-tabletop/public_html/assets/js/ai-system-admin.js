...
(function () {
  ...
  async function runTestConnection() {
    ...
    resultDiv.innerHTML = '<div class="text-center py-3"><div class="inline-spinner text-primary"></div><p class="mt-2">Sending test request...</p></div>';
  ...
  function refreshDefaultProviderRows(defaultProviderId) {
    ...
    rows.forEach((row) => {
      ...
      if (String(rowId) === String(defaultProviderId)) {
        defaultCell.innerHTML = '<span class="badge bg-primary"><i class="bi bi-check-circle mr-1"></i>Default</span>';
      } else {
  ...
  async function saveProviderConfig() {
    ...
    try {
      ...
      saveProviderBtn.innerHTML = '<i class="bi bi-save mr-1"></i>Save Changes';
    }
  }
  ...
  function checkOllamaStatus() {
    ...
    fetchProviderModels('ollama')
      .then((models) => {
        ...
        ollamaStatusEl.className = `badge ${online ? 'bg-success' : 'bg-secondary'} ml-2`;
        ollamaStatusEl.textContent = online ? 'Online' : 'Offline';
      })
      .catch(() => {
        ollamaStatusEl.className = 'badge bg-secondary ml-2';
