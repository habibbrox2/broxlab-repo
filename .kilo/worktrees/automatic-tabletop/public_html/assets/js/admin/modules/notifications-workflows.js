...
export async function initNotificationsSend() {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (submitBtn) {
          ...
          submitBtn.innerHTML = '<i class="bi bi-check mr-2"></i>Send Notification';
        }
      }
    } catch (error) {
      console.error('Error:', error);
  ...
  function setFilteredEmptyState(container, show) {
    ...
    if (!emptyState) {
      ...
      emptyState.innerHTML = '<i class="bi bi-search mr-1"></i>No recipients matched the selected filters';
  ...
  byId('previewBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        list.innerHTML = `<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-circle mr-2"></i>Recipient load error: ${ data.error }</div>`;
        updateFilteredMeta(0, 0);
      } else if (!data.recipients || data.recipients.length === 0) {
        if (totalCountEl) totalCountEl.textContent = '0';
        const warning = data.warning ? `<div class="alert alert-warning mb-2"><i class="bi bi-exclamation-triangle mr-2"></i>${data.warning}</div>` : '';
        list.innerHTML = `${warning }<div class="alert alert-info text-center mb-0"><i class="bi bi-info-circle mr-2"></i>No recipients found for this selection</div>`;
        updateFilteredMeta(0, 0);
      } else {
      ...
      if (totalCountEl) totalCountEl.textContent = '0';
      updateFilteredMeta(0, 0);
      byId('recipientList').innerHTML = `<div class="alert alert-danger mb-0"><i class="bi bi-exclamation-triangle mr-2"></i>Error: ${ error.message }</div>`;
    }
...
export async function initNotificationsScheduled() {
  ...
  async function submitScheduleForm(event) {
    ...
    if (submitBtn) {
      submitBtn.disabled = true;
      submitBtn.innerHTML = '<i class="bi bi-hourglass-split mr-2"></i>Scheduling...';
    }
    ...
    try {
      ...
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<i class="bi bi-check-lg mr-2"></i>Schedule';
      }
    }
  }
                    ...
                    <div class="col-lg-6 mb-4">
                        <div class="admin-panel-card h-100">
                            <div class="admin-panel-card-body d-flex flex-column">
                                ...
                                <div class="mb-3">
                                    <div class="mb-2">
                                        <small class="text-muted"><i class="bi bi-calendar mr-1"></i>Scheduled:</small>
                                        <div class="fw-bold">${escapeHtml(notif.scheduled_at || '-')}</div>
                                    </div>
                                    <div>
                                        <small class="text-muted"><i class="bi bi-people mr-1"></i>Recipient:</small>
        ...
        container.innerHTML = `
                    <div class="col-12">
                        <div class="alert alert-danger">
                            <i class="bi bi-exclamation-triangle mr-2"></i>Error: ${escapeHtml(error.message || 'Failed to load')}
                        </div>
                    </div>
                `;
      }
...
export function initNotificationsDeviceSync() {
  ...
  function getActionBadge(action) {
    const badges = {
      read: '<span class="badge bg-success"><i class="bi bi-check-circle mr-1"></i>Read</span>',
      dismissed: '<span class="badge bg-warning"><i class="bi bi-x-circle mr-1"></i>Dismissed</span>',
      deleted: '<span class="badge bg-danger"><i class="bi bi-trash mr-1"></i>Deleted</span>',
      sync: '<span class="badge bg-info"><i class="bi bi-arrow-repeat mr-1"></i>Sync</span>',
    };
        ...
        tbody.innerHTML = `
                    <tr>
                        <td colspan="5" class="text-center text-danger py-4">
                            <i class="bi bi-exclamation-triangle mr-2"></i>${escapeHtml(error.message || 'Failed')}
                        </td>
                    </tr>
                `;
      }
