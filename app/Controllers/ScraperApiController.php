...
if (!function_exists('scraperPushToNullableString')) {
    ...
    function scraperPushEnhanceIncomingPayload(mysqli $mysqli, string $dataType, array $payload): array
    {
        ...
        $userPrompt = "Clean and enhance the following {$dataType} payload. Return valid JSON only. Use this schema as a guide:\n"
            ...
            . "\nBANGLA LANGUAGE RULES (বাংলা ভাষার নিয়ম):\n"
            . "- Fix Bengali spelling errors: correct misspelled Bengali words (e.g. 'বাংলাদেশ' not 'বাংলাদেশ' with wrong vowel signs)\n"
            . "- Fix broken/corrupted Bengali text: restore garbled Unicode sequences to proper Bengali script\n"
            . "- Fix incorrect conjunct consonants (যুক্তাক্ষর): e.g. 'ক্ষ', 'ত্র', 'স্ত', 'ন্ত', 'ম্ব', 'ন্ন', 'ত্ত', 'দ্দ' etc. must be properly formed\n"
            . "- Fix wrong or missing hasanta (্), anusvar (ং), visarga (ঃ), chandrabindu (ঁ)\n"
            . "- Fix incorrect reph (র্) and ya-phala (্য), ra-phala (্র), la-phala (্ল) placements\n"
            . "- Fix wrong matras/vowel diacritics (া, ি, ী, ু, ূ, ে, ৈ, ো, ৌ) attached to wrong consonants\n"
            . "- Fix Unicode encoding issues: replace incorrectly encoded or visually similar wrong characters with correct Bengali Unicode codepoints\n"
            . "- Fix mixed script confusion: do not mix Assamese/other Indic characters where pure Bengali characters are expected\n"
            . "- Fix punctuation: use Bengali danda (।) for sentence endings where appropriate, not full stop (.)\n"
            . "- Fix incorrect use of zero-width joiner (ZWJ U+200D) or zero-width non-joiner (ZWNJ U+200C) that breaks conjuncts\n"
            . "- Preserve Bengali numerals (১২৩) or Arabic numerals (123) as found — do NOT convert between them\n"
            . "- Do NOT translate Bengali to English or English to Bengali\n"
            . "- Do NOT change the meaning, tone, or facts of Bengali content\n"
            . "\nFORBIDDEN changes:\n"
