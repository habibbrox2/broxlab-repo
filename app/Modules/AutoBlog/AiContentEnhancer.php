...
class AiContentEnhancer
{
    ...
    {
        $this->mysqli = $mysqli;
        $this->model = new AutoBlogModel($mysqli);
    ...
    private function enhanceContent(string $title, string $content, string $excerpt, string $sourceName): array
    {
        ...
        // Make API request
        $response = $this->callAiApi($endpoint, $model, $apiKey, $prompt);
...
PROMPT;
     ...
     * Call AI API
     */
    private function callAiApi(string $endpoint, string $model, string $apiKey, string $prompt): string
    {
        $headers = [
        ...
        if (curl_errno($ch)) {
            throw new \Exception('AI API connection error: ' . curl_error($ch));
        }
