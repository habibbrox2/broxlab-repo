...
function toBanglaDigits(value) {
  return String(value == null ? '' : value).replace(/[0-9]/g, (digit) => {
...
function renderWidgetError(root, message, tableBody) {
  ...
  if (tableBody) {
    tableBody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-red-600">${message}</td></tr>`;
  }
  ...
  if (!status && !title && root) {
    ...
    if (!existing) {
      ...
      div.className = 'ramadan-inline-error p-4 rounded-lg bg-amber-50 border-amber-200 text-amber-700 border mb-0';
...
function renderTable(tableBody, division, todayKey, mode, root) {
  ...
  try {
    ...
    const rows = SCHEDULE.map((rawEntry) => {
      ...
      return [
        ...
        `<td class="pl-4 font-semibold">${toBanglaDigits(entry.day)}</td>`,
    ...
    tableBody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-red-600">${RUNTIME_ERROR_TEXT}</td></tr>`;
  }
}
...
function initWidget(root) {
  ...
  try {
    ...
    const getWidgetTodayKey = function () {
      return resolveTodayKey(root);
    };

    const refreshStatic = function () {
      try {
    ...
    const refreshCountdown = function () {
      try {
...
export { initWidget, init, safeInit };
