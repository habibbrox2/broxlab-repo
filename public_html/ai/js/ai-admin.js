...
if (!window.BroxAdminInstance) {
  ...
  class FileAttachmentHandler {
    ...
    handleFiles(fileList) {
      ...
      if (file.size > ADMIN_CONFIG.maxFileSize) {
        window.showAlert(
          `File too large. Maximum size is ${ADMIN_CONFIG.maxFileSize / 1024 / 1024}MB`,
          'File Too Large',
          'warning'
        );
        return;
    ...
    uploadImage(file) {
      ...
      xhr.onload = () => {
        ...
        if (xhr.status < 200 || xhr.status >= 300 || !response.success) {
          ...
          reportTelemetry('upload_failure', {
            status: xhr.status,
            error: msg,
            url: ADMIN_CONFIG.uploadUrl,
          });
          return;
        }
  ...
  class BroxAdminCopilot {
    ...
    initVoiceInput() {
      ...
      try {
        ...
        const ensureMicAccess = async () => {
          ...
          self._micAccessPromise = navigator.mediaDevices
    ...
    updateModelLabel() {
      ...
      const rawLabel =
        this.nodes.modelSel?.options[this.nodes.modelSel.selectedIndex]?.text || modelId || 'AI';
    ...
    updateModelStatus(status) {
      ...
      this.nodes.modelStatusIndicator.classList.remove(
        'brox-ai-online',
        'brox-ai-offline',
        'brox-ai-connecting'
      );
    ...
    bindEvents() {
      ...
      if (this.nodes.providerSel) {
        this.nodes.providerSel.onchange = () => {
          ...
          if (hasImage && !this.isProviderMultimodal(provider)) {
            this.updateStatus(
              'warning',
              'Selected provider may not support images; results may be limited.'
            );
          }
        };
      }
      ...
      if (this.nodes.input) {
        ...
        this.nodes.input.addEventListener('input', (e) => {
          ...
          if (this.nodes.charCount) {
            ...
            this.nodes.charCount.classList.toggle(
              'brox-ai-warning',
              len > ADMIN_CONFIG.maxInputLength * 0.9
            );
          }
      ...
      document.addEventListener('pointerdown', (e) => {
        if (!this.nodes.modelBar || this.nodes.modelBar.classList.contains('brox-ai-collapsed'))
          return;
        const path = e.composedPath ? e.composedPath() : [];
        const clickedInside =
          this.nodes.modelBar &&
          (path.includes(this.nodes.modelBar) || this.nodes.modelBar.contains(e.target));
      ...
      document.addEventListener('pointerdown', (e) => {
        ...
        const clickedInside =
          path.includes(this.nodes.shell) ||
          path.includes(this.nodes.btn) ||
          this.nodes.shell.contains(e.target) ||
          this.nodes.btn.contains(e.target);
        if (clickedInside) return;
        this.closeSidebar();
      });
    ...
    toggleModelBar(forceState) {
      ...
      const willOpen =
        typeof forceState === 'boolean'
          ? forceState
          : this.nodes.modelBar.classList.contains('brox-ai-collapsed');
      if (willOpen) {
    ...
    pushHistoryState() {
      try {
        if (this.historyStateActive || (history.state && history.state[this.historyStateKey]))
          return;
    ...
    async handleSend() {
      ...
      if (text) {
        ...
        if (!validation.valid) {
          ...
          this.addMessage('system', `❌ ${validation.error}`, false);
        ...
        if (
          urlToBrowse &&
          (sanitized.toLowerCase().includes('browse') ||
            sanitized.toLowerCase().includes('open') ||
            sanitized.toLowerCase().includes('fetch') ||
            sanitized.toLowerCase().includes('extract') ||
            sanitized.toLowerCase().includes('get content') ||
            sanitized.toLowerCase().includes('what is on') ||
    ...
    async regenerateAssistantMessage(msgIndex) {
      ...
      if (
        msgIndex < 0 ||
        msgIndex >= this.history.length ||
        this.history[msgIndex]?.role !== 'assistant'
      )
        return;
    ...
    createAssistantActions({ msgEl, contentEl, metaEl, msgIndex = null }) {
      ...
      regenBtn?.addEventListener('click', async (e) => {
        e.stopPropagation();
        const idx = Number.isInteger(msgIndex)
          ? msgIndex
          : Number.parseInt(msgEl?.dataset?.msgIndex || '-1', 10);
    ...
    addMessage(role, content, animate = true, msgIndex = null) {
      ...
      if (Array.isArray(content)) {
        content.forEach((part, idx) => {
          ...
          if (part.type === 'image_url' && part.image_url && part.image_url.url) {
            ...
            img.addEventListener('click', () =>
              this.showImageLightbox(part.image_url.url, part.image_url.name || 'Image')
            );
        ...
        content.includes(String.fromCharCode(96).repeat(3))
      ) {
      ...
      if (role === 'assistant') {
        this.attachLongPressCopy(
          contentDiv,
          () => contentDiv.innerText || this.getMessagePlainText(content)
        );
      }
    ...
    normalizeAutoFillPayload(payload) {
      ...
      if (Array.isArray(payload)) {
        ...
        payload.forEach((field) => {
          ...
          const value =
            field.value ?? field.val ?? field.content ?? field.text ?? field.answer ?? '';
          if (!name) return;
    ...
    extractJsonCandidates(text) {
      ...
      const artifactRegex = /```artifact\s*([\s\S]*?)```/gi;
      let match = null;
      ...
      const anyFenceRegex = /```(?:[a-zA-Z0-9_-]+)?\s*([\s\S]*?)```/gi;
    ...
    getAutoFillKeyScore(payload) {
      ...
      const formHits = formLikeKeys.reduce(
        (count, key) => count + (lowerKeys.includes(key) ? 1 : 0),
        0
      );
    ...
    applyAutoFillPayload(payload, form) {
      ...
      keys.forEach((key) => {
        ...
        if (
          ...
          typeof window.editor_content.setContent === 'function'
        ) {
    ...
    getMultimodalProvider() {
      ...
      return (
        Object.keys(this.providerMeta).find(
          (provider) => this.providerMeta[provider]?.supports_multimodal
        ) || null
      );
    }
    ...
    async ensureMultimodalProviderForInput(hasImageContent) {
      ...
      if (!multimodalProvider) {
        this.updateStatus(
          'warning',
          'No multimodal-capable provider configured (images may not be properly processed).'
        );
        return;
      }
    ...
    async handleCollectData() {
      ...
      if (hasAttachment && attachment?.isImage) {
        ...
        try {
          ...
          if (!ocrResult.success || !ocrResult.text) {
            this.addMessage(
              'assistant',
              `❌ OCR failed: ${ocrResult.error || 'No text found in image'}`
            );
            this.updateStatus('ready', 'Ready');
            return;
          }
      ...
      if (!formData || !formData.fields.length) {
        this.addMessage(
          'assistant',
          'Form found, but no form fields were detected or they are empty.'
        );
        return;
      }
    ...
    async applyAutoFillFromLastAssistant() {
      ...
      if (!payload || typeof payload !== 'object') {
        ...
        this.addMessage(
          'assistant',
          'Could not find structured JSON in the last assistant response to auto-fill the form.'
        );
        return;
      }
    ...
    async handleAnalyzeLogs() {
      try {
        ...
        if (data.errors && data.errors.length > 0) {
          ...
          const latestErrors = data.errors
            .slice(0, 5)
        ...
        this.addMessage(
          'assistant',
          '❌ Failed to fetch logs. Please ensure you have admin permissions.'
        );
      }
      this.saveHistory();
    }
    ...
    async handleGenerateReport() {
      try {
        ...
        if (data.success && data.summary) {
          ...
          this.addMessage(
            'assistant',
            '📊 Report generated with default data. Detailed analytics available in dashboard.'
          );
        }
      } catch (e) {
    ...
    async handleSearchKB() {
      ...
      try {
        const res = await fetch(
          `/api/admin/ai-knowledge/search?q=${encodeURIComponent(query)}&limit=3`,
          {
    ...
    async handleNavigationRequest(text, pageName) {
      ...
      if (!url) {
        this.addMessage(
          'assistant',
          `I couldn't recognize the page "${pageName || text}". Try: dashboard, users, posts, settings, analytics, notifications, media, comments, security, permissions, roles, pages, themes, logs, profile, services.`
        );
        this.saveHistory();
        return;
      }
    ...
    showNotificationBadge(count, type = 'error') {
      ...
      const triggerBtn =
        document.querySelector('#adminAiShell .ai-trigger-btn') ||
    ...
    async autoSelectModel() {
      ...
      const hasComplexTask = complexKeywords.some((kw) => queryText.toLowerCase().includes(kw));
    ...
    async getAIResponse() {
      ...
      if (!fullReply) {
        const msg = lastError
          ? lastError.message || 'AI error. Please try again.'
          ...
          if (Array.isArray(data.errors)) {
            ...
            if (count > 0) {
              ...
              if (count > lastErrorCount) {
                this.addMessage(
                  'assistant',
                  `⚠️ System Alert: ${count} new error(s) detected in logs.`
                );
              }
            }
  ...
  function bootstrapAdminCopilot() {
    ...
    window.deleteProvider = (id) =>
      window.broxAdmin.apiCall('/api/ai-system/delete-provider', { id });
  }
