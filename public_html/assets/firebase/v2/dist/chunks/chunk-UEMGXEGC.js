...
// public_html/assets/firebase/v2/config.js
var MAX_CACHE_AGE_MS = 7 * 24 * 60 * 60 * 1e3;
var _cachedConfig = null;
var _fetchInProgress = null;
var _notificationConfig = null;
var DEFAULT_NOTIFICATION_CONFIG = {
  ...
  notifications: {
    ...
    maxNotifications: 10,
    // max concurrent notifications on screen
    popupDelay: 300,
    // ms before showing popup
    popupCooldown: 500,
    // ms between popup displays
    autoCloseDelay: 5e3,
    // ms before auto-closing toast
  ...
  scheduledNotifications: {
    enabled: true,
    checkInterval: 6e4,
    // ms between server checks
    retentionDays: 30,
    // keep history for N days
    timezoneAware: true,
    maxScheduledPerUser: 100,
    enableClientSideScheduler: false
  ...
  multiDeviceSync: {
    enabled: true,
    deduplication: true,
    deduplicateWindow: 5e3,
    // ms window for dedup
    syncInterval: 3e4,
    // ms between syncs
    syncOnForeground: true,
    // sync when tab comes to foreground
    maxDevicesPerUser: 5,
  ...
  offlineHandling: {
    ...
    retryInterval: 5e3,
    // start retry after this delay
    maxRetries: 5,
    retryDelayMultiplier: 1.5,
    // exponential backoff: 5s, 7.5s, 11.25s...
    enableBackgroundSync: true,
    syncReadStatus: true,
    // sync read/unread status offline
  ...
  tokenManagement: {
    ...
    // ms (1 day) - clean up expired tokens
    heartbeatSyncIntervalMs: 864e5,
    // ms (1 day) - refresh server timestamp even when token is unchanged
    heartbeatCheckIntervalMs: 9e5,
    // ms (15 min) - check whether heartbeat sync is due
    tokenExpiryDays: 90,
    validateTokenOnStartup: true,
    maxTokensPerDevice: 5,
    maxTokenRetries: 4,
    maxSyncRetries: 3,
    tokenValidationRegex: "^[A-Za-z0-9_:-]+$",
    // FCM token format
    minTokenLength: 10,
    syncDebounceMs: 3e3,
    // debounce token sync requests
  ...
  rateLimiting: {
    enabled: true,
    minLoadInterval: 1e3,
    // ms between config loads
    maxRetries: 3,
    // max retries before giving up
    retryDelayMultiplier: 2,
    // exponential backoff multiplier
    requestTimeout: 1e4,
    // ms timeout for network requests
    enableAdaptiveThrottling: true
    // adjust rates based on network
  },
...
async function loadFirebaseConfig(timeout = 5e3) {
  ...
  _fetchInProgress = (async () => {
    ...
    try {
      ...
      if (ok) {
        ...
        if (validated) {
          try {
            localStorage.setItem("firebase_config_cache", JSON.stringify({ ts: Date.now(), config: validated }));
          } catch (e) {
          }
          _cachedConfig = validated;
          return validated;
  ...
  try {
    const r = await _fetchInProgress;
    return r;
  } finally {
...
async function getFullConfig(timeout = 5e3) {
...
async function _ensureNotificationConfig() {
  if (_notificationConfig) return _notificationConfig;
  await loadFirebaseConfig().catch(() => null);
  try {
...
async function prefetchAllConfigs() {
  try {
    await Promise.all([
...
function updateEmulatorBanner(name8, isRunningEmulator) {
  if (typeof window === "undefined" || typeof document === "undefined" || !isCloudWorkstation(window.location.host) || emulatorStatus[name8] === isRunningEmulator || emulatorStatus[name8] || // If already set to use emulator, can't go back to prod.
  previouslyDismissed) {
    return;
  }
...
function openDB(name8, version8, { blocked, upgrade, blocking, terminated } = {}) {
...
function deleteDB(name8, { blocked } = {}) {
  const request = indexedDB.deleteDatabase(name8);
  if (blocked) {
    request.addEventListener("blocked", (event) => blocked(
      // Casting due to https://github.com/microsoft/TypeScript-DOM-lib-generator/pull/1405
      event.oldVersion,
...
function initializeApp(_options, rawConfig = {}) {
  ...
  if (existingApp) {
    if (deepEqual(options, existingApp.options) && deepEqual(config, existingApp.config)) {
      ...
      throw ERROR_FACTORY.create("duplicate-app", { appName: name8 });
    }
  }
...
function getApp(name8 = DEFAULT_ENTRY_NAME2) {
  const app = _apps.get(name8);
  if (!app && name8 === DEFAULT_ENTRY_NAME2 && getDefaultAppConfig()) {
    return initializeApp();
  }
  if (!app) {
    throw ERROR_FACTORY.create("no-app", { appName: name8 });
  }
  return app;
}
function getApps() {
...
var PersistenceUserManager = class _PersistenceUserManager {
  constructor(persistence, auth, userKey) {
    ...
    const { config, name: name8 } = this.auth;
...
var CookiePersistence = class {
  ...
  async _get(key) {
    ...
    if (window.cookieStore) {
      const cookie = await window.cookieStore.get(name8);
      return cookie?.value;
    }
...
function _open(auth, url, name8, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) {
  ...
  if (name8) {
    target = _isChromeIOS(ua) ? TARGET_BLANK : name8;
  }
  if (_isFirefox(ua)) {
...
var ERROR_FACTORY4 = new ErrorFactory("analytics", "Analytics", ERRORS2);
...
async function validateIndexedDB() {
  if (!isIndexedDBAvailable()) {
    logger2.warn(ERROR_FACTORY4.create("indexeddb-unavailable", {
...
var ERROR_FACTORY5 = new ErrorFactory("remoteconfig", "Remote Config", ERROR_DESCRIPTION_MAP2);
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    try {
      ...
      reject(ERROR_FACTORY5.create("storage-open", {
...
var Storage = class {
  ...
  setActiveConfigTemplateVersion(version8) {
    return this.set("last_known_template_version", version8);
  }
};
...
function mergeCustomSignals(customSignals, storedSignals) {
  ...
  if (Object.keys(updatedSignals).length > RC_CUSTOM_SIGNAL_MAX_ALLOWED_SIGNALS) {
    throw ERROR_FACTORY5.create("custom-signal-max-allowed-signals", {
...
var RealtimeHandler = class {
  constructor(firebaseInstallations, storage, sdkVersion, namespace, projectId, apiKey, appId, logger3, storageCache, cachingClient) {
    this.firebaseInstallations = firebaseInstallations;
    this.storage = storage;
    this.sdkVersion = sdkVersion;
    this.namespace = namespace;
  ...
  async fetchLatestConfig(remainingAttempts, targetVersion) {
    ...
    try {
      ...
      const error = ERROR_FACTORY5.create("update-not-fetched", {
      ...
      if (partialConfigUpdateMessage.includes("}")) {
        ...
        try {
          ...
          if (REALTIME_DISABLED_KEY in jsonObject && jsonObject[REALTIME_DISABLED_KEY] === true) {
            const error = ERROR_FACTORY5.create("realtime-unavailable", {
          ...
          const errorMessage = e instanceof Error ? e.message : String(e);
          this.propagateError(ERROR_FACTORY5.create("update-message-invalid", {
  ...
  async prepareAndBeginRealtimeHttpStream() {
    ...
    try {
      ...
      if (connectionFailed || response?.ok) {
        ...
        const firebaseError = ERROR_FACTORY5.create("stream-error", {
          originalErrorMessage: errorMessage
        });
  ...
  async makeRealtimeHttpConnection(delayMillis) {
    ...
    if (this.httpRetriesRemaining > 0) {
      ...
      const error = ERROR_FACTORY5.create("stream-error", {
...
function registerRemoteConfig() {
  ...
  registerVersion(name7, version7, "esm2020");
  function remoteConfigFactory(container, { options }) {
    ...
    const logger3 = new Logger(name7);
    ...
    const realtimeHandler = new RealtimeHandler(installations, storage, SDK_VERSION, namespace, projectId, apiKey, appId, logger3, storageCache, cachingClient);
    const remoteConfigInstance = new RemoteConfig(app, cachingClient, storageCache, storage, logger3, realtimeHandler, analyticsProvider);
...
// public_html/assets/firebase/v2/init.js
var _app = null;
var _modules = null;
var _initialized = false;
var _messagingSupportPromise = null;
var _autoInitPromise = null;
...
async function resolveMessagingSupport() {
  ...
  _messagingSupportPromise = (async () => {
    ...
    try {
      return await isWindowSupported() === true;
    } catch (err) {
      return false;
    }
  })();
...
async function initFirebase(options = {}) {
  ...
  const cfg = await loadFirebaseConfig(options.configTimeout || 5e3).catch((e) => {
  ...
  try {
    ...
    _modules = {
      ...
      messaging: messagingSupported ? (() => {
        try {
          return getMessagingInWindow(_app);
        } catch (e) {
          return null;
        }
      })() : null,
...
function getAuthInstance() {
  return _modules?.auth || null;
}
function getMessagingInstance() {
  return _modules?.messaging || null;
}
function getAnalyticsInstance() {
  return _modules?.analytics || null;
}
function getRemoteConfigInstance() {
  return _modules?.remoteConfig || null;
}
async function getFirebaseConfig(timeout = 5e3) {
  return await loadFirebaseConfig(timeout);
}
function FirebaseModules() {
  return _modules;
}
function getFirebaseInitPromise() {
  return _autoInitPromise;
}
