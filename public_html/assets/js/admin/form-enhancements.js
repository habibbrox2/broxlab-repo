 ...
 * ES Module — bundles through esbuild
 */
...
const updateSaveIndicator = () => {
  ...
  if (state.isDirty && !state.lastSaved) {
    indicator.innerHTML = '<i class="lucide lucide-edit text-amber-500"></i> Unsaved changes';
    indicator.classList.remove('opacity-0');
  } else if (state.lastSaved) {
    indicator.innerHTML = `<i class="lucide lucide-check text-green-500"></i> Last saved ${formatTime(state.lastSaved)}`;
    indicator.classList.remove('opacity-0');
...
// Initialize on DOM ready (deferred modules execute after HTML parse, so DOMContentLoaded fires normally)
document.addEventListener('DOMContentLoaded', init);

// Expose for backward compatibility (e.g. inline scripts in form.twig)
const PostFormEnhancements = { init, validateField, };
window.PostFormEnhancements = PostFormEnhancements;

export { init, validateField };
