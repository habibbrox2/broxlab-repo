...
function renderLoadingTableRows(columnCount = 6, rowCount = 5) {
  const cellHtml = '<td class="text-muted small py-3"><span class="inline-spinner inline-spinner-sm text-primary mr-2" role="status" aria-hidden="true"></span>Loading...</td>';
...
function renderBrandSectionsLoading() {
  document.querySelectorAll('.medex-section-content').forEach((el) => {
    el.innerHTML = '<div class="d-flex align-items-center gap-2 text-muted py-3"><span class="inline-spinner inline-spinner-sm text-primary" role="status" aria-hidden="true"></span> Loading details...</div>';
...
function showRouteStatus(id, message, isError = false, showSpinner = false) {
  ...
  if (isError) {
    ...
    el.innerHTML = `<span class="inline-spinner inline-spinner-sm text-primary mr-1" role="status" aria-hidden="true"></span> ${escapeHtml(message)}`;
  } else {
...
function showRouteToast(message, type) {
  ...
  if (type === 'success') {
    ...
    iconHtml = '<i class="bi icon-check-circle" style="font-size:1.1rem;"></i>';
    dismissAfter = 5000;
  } else if (type === 'error') {
    bgClass = 'medex-toast--danger';
    iconHtml = '<i class="bi icon-alert-circle" style="font-size:1.1rem;"></i>';
    dismissAfter = 8000;
  }
          ...
          <tr class="medex-hover-lift">
            <td>
              <a href="/medex/brand/${encodeURIComponent(brand._id)}" class="text-decoration-none fw-semibold medex-brand-name">
                <i class="bi icon-pill mr-1 text-success"></i>${escapeHtml(brand.name || 'Unknown')}
              </a>
            </td>
            ...
            <td class="text-center">
              <a href="/medex/brand/${encodeURIComponent(brand._id)}" class="medex-btn medex-btn--details">
                <i class="bi icon-file-plus"></i> View
              </a>
            </td>
          </tr>`;
    })
            ...
            <tr class="medex-hover-lift">
              ...
              <td>
                <a href="${companyUrl}" class="company-name text-decoration-none">
                  ...
                  ${headquarter ? `<br><small class="text-muted"><i class="bi icon-map-pin mr-1"></i>${headquarterText}</small>` : ''}
                </a>
              </td>
              ...
              <td class="text-center"><a href="${companyUrl}" class="medex-btn medex-btn--view"><i class="bi icon-eye"></i> View</a></td>
            </tr>`;
    })
    .join('');
}
