...
function updateHfTips(_providerName) {
  ...
  if (hfTipsFrontend && frontendProviderSelect) {
    hfTipsFrontend.classList.toggle('hidden', frontendProviderSelect.value !== 'huggingface');
  }
  if (hfTipsBackend && backendProviderSelect) {
    hfTipsBackend.classList.toggle('hidden', backendProviderSelect.value !== 'huggingface');
  }
}
...
function renderModelOptions(selectEl, models, savedModel, providerName, warningEl, refreshBtn, usedFallback, fetchError) {
  ...
  if (!models.length) {
    ...
    if (warningEl) {
      ...
      warningEl.classList.remove('text-red-600', 'text-amber-600');
  ...
  if (warningEl) {
    if (providerName === 'huggingface' && hiddenNonChat) {
      ...
      warningEl.classList.add('text-amber-600');
      warningEl.classList.remove('hidden');
    } else if (usedFallback) {
...
async function runTestConnection() {
  ...
  resultDiv.innerHTML = '<div class="flex flex-col items-center gap-2 py-3 text-slate-600"><div class="inline-spinner text-indigo-600" role="status"></div><p class="mt-2">Sending test request...</p></div>';
...
async function toggleProviderMultimodal(inputEl) {
  ...
  try {
    ...
    if (badge) {
      badge.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${enabled ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`;
...
function updateProviderRowState(rowEl, isActive) {
  ...
  if (badge) {
    badge.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isActive ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`;
...
function refreshDefaultProviderRows(defaultProviderId) {
  ...
  rows.forEach((row) => {
    ...
    if (String(rowId) === String(defaultProviderId)) {
      defaultCell.innerHTML = '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-600 inline-flex items-center gap-1"><i class="lucide lucide-check-circle"></i>Default</span>';
    } else {
      const disabledAttr = isActive ? '' : 'disabled';
      defaultCell.innerHTML = `<button class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-lg border border-indigo-600 text-indigo-600 hover:bg-indigo-50 transition-colors set-default-btn" data-provider-id="${rowId}" ${disabledAttr}>Set as Default</button>`;
    }
  });
...
function updateStatusPills(providerName) {
  ...
  if (apiKeyPill) {
    apiKeyPill.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${provider.has_api_key ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`;
    apiKeyPill.textContent = provider.has_api_key ? 'API Key: Present' : 'API Key: Missing';
  }

  if (activePill) {
    activePill.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${provider.is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`;
    activePill.textContent = provider.is_active ? 'Provider: Active' : 'Provider: Inactive';
  }

  if (testedPill) {
    testedPill.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-sky-100 text-sky-800';
...
function checkOllamaStatus() {
  ...
  fetchProviderModels('ollama')
    .then((models) => {
      ...
      ollamaStatusEl.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${online ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'} ml-2`;
      ollamaStatusEl.textContent = online ? 'Online' : 'Offline';
    })
    .catch(() => {
      ollamaStatusEl.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-200 text-slate-700 ml-2';
