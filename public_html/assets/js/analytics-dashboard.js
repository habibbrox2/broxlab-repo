/**
 * analytics-dashboard.js (ES Module)
...
function updateSecurityAlertsDOM(alerts) {
  ...
  if (!alerts || alerts.length === 0) {
    container.innerHTML = '<div class="p-4 rounded-lg bg-emerald-50 border-emerald-200 text-emerald-700 border">✓ No security alerts in the last 24 hours</div>';
    return;
  }
  ...
  alerts.slice(0, 5).forEach(alert => {
    ...
    html += `
                <div class="alert ${severityClass === 'danger' ? 'bg-red-50 border-red-200 text-red-700' : severityClass === 'warning' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-sky-50 border-sky-200 text-sky-700'} border" role="alert">
                    <strong>${icon} ${(alert.type || 'ALERT').toUpperCase()}</strong>
                    <br/>
                    ${alert.message || 'Security event detected'}
                    <small class="block mt-2 text-slate-500">${alert.timestamp || new Date().toLocaleString()}</small>
...
function updateLoginAuditDOM(logs) {
  ...
  if (!logs || logs.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="text-center text-slate-500">No login activity</td></tr>';
    return;
  }
  ...
  logs.forEach(log => {
    ...
    html += `
                <tr>
                    <td>${log.user_id || '-'}</td>
                    <td>${status}</td>
                    <td><code class="text-xs">${log.ip_address || 'Unknown'}</code></td>
                    <td><small class="text-slate-500">${truncateText(log.user_agent || 'Unknown', 30)}</small></td>
                    <td><small class="text-slate-500">${formatDate(log.created_at)}</small></td>
                </tr>
            `;
  });
...
function updateOAuthAuditDOM(logs) {
  ...
  if (!logs || logs.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="text-center text-slate-500">No OAuth activity</td></tr>';
    return;
  }
  ...
  logs.forEach(log => {
    ...
    html += `
                <tr>
                    <td>${log.user_id || '-'}</td>
                    <td><code class="text-xs">${log.action || 'Unknown'}</code></td>
...
function updateActivityLogsDOM(logs) {
  ...
  if (!logs || logs.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="text-center text-slate-500">No activity recorded</td></tr>';
    return;
  }
  ...
  logs.forEach(log => {
    html += `
                <tr>
                    <td><code class="text-xs">${log.action_type || 'Unknown'}</code></td>
...
function startActivityLogsPolling() {
  ...
  state.activityPollingId = setInterval(async () => {
    try {
      const data = await safeFetchJson(`${API_BASE}/activity-logs?limit=20`);
      if (data && data.success && data.data && data.data.length > 0) {
        updateActivityLogsDOM(data.data);
...
function setupEventListeners() {
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      if (typeof broxUI !== 'undefined') {
        const modal = new broxUI.Modal(document.querySelector('#exportModal'));
        modal.show();
      }
    });
  }
  ...
  if (dailyTab) {
    dailyTab.addEventListener('brox:shown', () => {
  ...
  if (weeklyTab) {
    weeklyTab.addEventListener('brox:shown', () => {
  ...
  if (monthlyTab) {
    monthlyTab.addEventListener('brox:shown', () => {
  ...
  if (loginAuditTab) {
    loginAuditTab.addEventListener('brox:shown', () => {
  ...
  if (oauthAuditTab) {
    oauthAuditTab.addEventListener('brox:shown', () => {
  ...
  if (activityLogsTab) {
    activityLogsTab.addEventListener('brox:shown', () => {
...
function renderDailyReports() {
  ...
  const dailyLogins = dashboardData?.daily_logins || [];
...
function renderWeeklyReports() {
  ...
  const weeklyStats = dashboardData?.weekly_stats || [];
