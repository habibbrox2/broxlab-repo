...
function setGenerating(state) {
  ...
  generateBtn.innerHTML = state
    ...
    : '<i class="bi icon-wand-sparkles mr-1"></i> <span id="generateBtnText">Generate Article</span>';
}
...
function setPublishing(state) {
  ...
  publishBtn.innerHTML = state
    ...
    : '<i class="bi icon-globe mr-1"></i> Publish Now';
  saveDraftBtn.innerHTML = state
    ? '<span class="loading-spinner"></span> Saving\u2026'
    : '<i class="bi icon-save mr-1"></i> Save as Draft';
}
...
function renderRecent(items) {
  ...
  recentList.innerHTML = items.map((item) => {
    return `<button type="button" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" data-title="${esc(item.title)}">` +
        ...
        '<i class="bi icon-arrow-right text-primary"></i></button>';
  }).join('');
...
function renderArticle(article) {
  ...
  if (tags.length) {
    articleTags.innerHTML = `<strong class="d-block mb-1 small text-uppercase text-muted">Tags</strong>${tags.map((t) => { return `<span class="tag-badge">${esc(t)}</span>`; }).join('')}`;
  ...
  if (kp.length) {
    ...
    articleKeyPoints.innerHTML = '<h6 class="fw-semibold mb-2"><i class="bi icon-list-checks mr-1"></i>Key Points</h6>' +
...
function saveArticle(publish) {
  ...
  fetch('/api/admin/ai/article-writer/publish', {
    ...
    .then((d) => {
      ...
      editBtn.innerHTML = '<i class="bi icon-external-link mr-1"></i> Continue in Editor';
      if (publish) { publishBtn.innerHTML = '<i class="bi icon-check-circle mr-1"></i> Published'; publishBtn.disabled = true; }
      else { saveDraftBtn.innerHTML = '<i class="bi icon-check-circle mr-1"></i> Draft Saved'; saveDraftBtn.disabled = true; }
    })
    .catch((err) => {
      setPublishing(false);
