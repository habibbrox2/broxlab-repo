...
const MIN_CACHE_TTL = 5 * 60 * 1000; // 5 minutes minimum
...
export class ModelCache {
     ...
     */
    async prefetch(providers = [], options = {}) {
 ...
 */
export async function initializeModelCache(providers = [], options = {}) {
    ...
    if (providers.length > 0) {
        cache.prefetch(providers, { skipCache: false })
            .then(() => {
                console.debug('[ModelCache] Prefetch completed');
            })
            .catch(err => {
