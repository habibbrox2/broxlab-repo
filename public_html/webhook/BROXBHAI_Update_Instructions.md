...
2. Site files backup → `storage/backups/webhook/site_<project>_*`
3. Database backup → (optional, if you implement DB backup support)
4. `git pull` চলবে
...
3. **Save** করুন
4. Test: `action=status` call করার সময় `X-Api-Key` header ব্যবহার করুন (URL-এ `api_key=` আর কাজ করবে না)

**Note (Production):** Admin actions চালু করতে server `.env`-এ `WEBHOOK_ADMIN_ACTIONS_ENABLED=1` সেট করতে হবে।
...
```
curl -H "X-Api-Key: YOUR_KEY" "https://yourdomain.com/webhook/github.php?action=versions"
```
...
```
curl -H "X-Api-Key: YOUR_KEY" "https://yourdomain.com/webhook/github.php?action=rollback&version=v20240114090000"
```
...
```bash
# Site backup দেখুন
ls -lh storage/backups/webhook/site_*/

# DB backup দেখুন
# ls -lh /path/to/db_backups/*.sql
```

### পুরনো Backup মুছুন (Disk Space বাঁচাতে)
```bash
# সব site backup দেখুন
ls storage/backups/webhook/ | grep site_

# নির্দিষ্ট backup মুছুন
rm -rf storage/backups/webhook/site_<project>_2024-01-10_120000

# নির্দিষ্ট DB backup মুছুন
# rm /path/to/db_backups/site<project>_2024-01-10_120000.sql
```

### DB Backup থেকে Restore করুন
```bash
# mysql -u your_db_user -p your_db_name < /path/to/db_backups/site<project>_2024-01-15_120000.sql
```
...
du -sh storage/backups/webhook/

# Site backup আলাদাভাবে
du -sh storage/backups/webhook/site_*/

# DB backup আলাদাভাবে
# du -sh /path/to/db_backups/
```
