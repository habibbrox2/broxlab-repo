...
/**
 * Teletalk Scraper Cron Job
 * Automated scraping of Teletalk government jobs
 * 
 * Usage: php scripts/cron/teletalk-scraper.php [--max-pages=N] [--verbose]
 * 
 * Cron setup (add to crontab):
 * 0 6 * * * /usr/bin/php /path/to/broxlab/scripts/cron/teletalk-scraper.php >> /path/to/logs/teletalk-scraper.log 2>&1
 */
...
logMessage("=== Teletalk Scraper Started ===", true);
...
try {
    ...
    logMessage("New jobs added: {$stats['new_jobs']}", true);
    ...
    if ($stats['new_jobs'] > 0) {
        ...
        $message = "<p>The Teletalk scraper found <strong>{$stats['new_jobs']}</strong> new jobs.</p>";
        $message .= "<ul>";
