...
return [
    'dedup' => [
        ...
        'similarity' => (float)($_ENV['AUTOBLOG_DEDUP_SIMILARITY'] ?? 0.8),
    ],
    'proxies' => [
        'enabled' => filter_var($_ENV['AUTOBLOG_PROXY_ENABLED'] ?? false, FILTER_VALIDATE_BOOLEAN),
        // comma-separated list of proxy URLs, e.g. http://user:pass@host:port
        'list' => array_filter(array_map('trim', explode(',', $_ENV['AUTOBLOG_PROXY_LIST'] ?? ''))),
    ],
    'cron' => [
        'max_articles_per_source' => (int)($_ENV['AUTOBLOG_MAX_ARTICLES_PER_SOURCE'] ?? 10),
        'max_sources_per_run' => (int)($_ENV['AUTOBLOG_MAX_SOURCES_PER_RUN'] ?? 20),
        'log_path' => __DIR__ . '/../storage/logs/autoblog_worker.log',
    ],
    'telegram' => [
    ...
    'api' => [
        ...
        'token' => $_ENV['AUTOBLOG_API_TOKEN'] ?? '',
        'max_limit' => (int)($_ENV['AUTOBLOG_API_MAX_LIMIT'] ?? 100),
    ],
];
