...
if (!function_exists('brox_resolve_asset_for_development')) {
    function brox_resolve_asset_for_development(string $url): string
    {
        ...
        $buildMap = [
            ...
            '/assets/ai/dist/' => [
                'watchRoots' => [
                    $projectRoot . '/public_html/assets/ai/js',
                    $projectRoot . '/public_html/assets/ai/css',
                    $projectRoot . '/build/esbuild-ai.mjs',
                ],
                'distRoot' => $projectRoot . '/public_html/assets/ai/dist',
                'commandKey' => 'ai',
...
if (!function_exists("registerTwigHelpers")) {
    function registerTwigHelpers(\Twig\Environment $twig, mysqli $mysqli, array &$session, array $appSettings, string $configUrl): void
    {
        ...
        $twig->addFilter(new \Twig\TwigFilter('role_badge', function ($roleName) {
            $badges = [
                'super_admin' => '<span class="badge badge-danger"><i class="lucide lucide-shield mr-1"></i>Super Admin</span>',
                'admin' => '<span class="badge badge-warning"><i class="lucide lucide-shield-check mr-1"></i>Admin</span>',
                'moderator' => '<span class="badge badge-info"><i class="lucide lucide-shield mr-1"></i>Moderator</span>',
                'user' => '<span class="badge badge-secondary"><i class="lucide lucide-user mr-1"></i>User</span>',
            ];
            return $badges[$roleName] ?? '<span class="badge badge-light text-dark">' . ucfirst($roleName) . '</span>';
        }, ['is_safe' => ['html']]));
        ...
        // Absolute URL filter - converts relative paths to absolute URLs for SEO and social sharing
        ...
        $twig->addFunction(new \Twig\TwigFunction('asset', function ($path, $version = true) {
            ...
            if (brox_is_development_env() && strpos($url, '/cdn/') === 0) {
                static $cdnMap = [
                    '/cdn/css/lucide/lucide.css'       => 'https://cdn.jsdelivr.net/npm/lucide-static@latest/font/lucide.css',
        ...
        // Add base URL global - use absolute URL for SEO, meta tags, and canonical URLs
        ...
        // Inject full JSON translations for client-side use
        $twig->addGlobal('site_translations', [
            'en' => LanguageHelper::getTranslations('en'),
            'bn' => LanguageHelper::getTranslations('bn'),
        ]);

        $twig->addGlobal('app_settings', $appSettings);
        $twig->addGlobal('is_dev_env', brox_is_development_env());
        $publicNavItems = [
            ['label' => 'Home', 'url' => '/', 'icon' => 'house', 'match' => '/', 'enabled' => true, 'order' => 10],
            ['label' => 'Mobiles', 'url' => '/mobiles', 'icon' => 'smartphone', 'match' => '/mobiles', 'enabled' => true, 'order' => 20],
            ['label' => 'Articles', 'url' => '/posts', 'icon' => 'book-open', 'match' => '/posts', 'enabled' => true, 'order' => 30],
            ['label' => 'Services', 'url' => '/services', 'icon' => 'award', 'match' => '/services', 'enabled' => true, 'order' => 40],
        ];
