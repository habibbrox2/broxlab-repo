{{--
    Port of legacy public/partials/weather-widget.twig — home weather section.
    JS fetches GET /weather/details?location=...&format=json (Laravel port) and
    renders the widget; geolocation with Dhaka fallback, exactly like legacy.
--}}
<section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 home-weather-section py-8" id="weather-section">
...
document.addEventListener('DOMContentLoaded', function() {
    var searchForm = document.getElementById('home-weather-search-form');
    var searchInput = document.getElementById('home-weather-search-input');
    var container = document.getElementById('weather-widget-container');
    var weatherDetailsBaseUrl = '/weather/details';
    ...
    function loadWeatherForUserLocation() {
        var fallbackLocation = 'Dhaka';

        if (!navigator.geolocation) { loadWeatherData(fallbackLocation); return; }
        ...
        try {
            if (navigator.permissions && typeof navigator.permissions.query === 'function') {
                navigator.permissions.query({ name: 'geolocation' }).then(function (permissionStatus) {
                    if (permissionStatus.state === 'denied') { loadWeatherData(fallbackLocation); return; }
                    requestPosition();
        ...
        function requestPosition() {
            navigator.geolocation.getCurrentPosition(
                ...
                function() { loadWeatherData(fallbackLocation); },
    ...
    async function loadWeatherData(location) {
        ...
        try {
            var url = new URL(weatherDetailsBaseUrl, window.location.origin);
    ...
    async function parseJsonResponse(response) {
        ...
        try { return JSON.parse(cleanText); } catch (error) {
            console.error('Failed to parse weather JSON response:', {error: error, text: text});
            return {};
        }
    }
    ...
    function renderWeatherWidget(data) {
        ...
        var locationName = current.location_name || 'Selected Location';
        var temp = Math.round(current.temperature || 0);
        var feels = Math.round(current.feels_like || temp);
        var humidity = current.humidity || 0;
        var wind = current.wind_speed || 0;
        var windDeg = current.wind_deg || 0;
        var pressure = current.pressure || 0;
        var description = (current.description || 'Clear').toLowerCase();
        var icon = current.icon || '01d';
        var unit = '°C';

        var windDir = getWindDirection(windDeg);
        var iconClass = getBootstrapIcon(icon);
        var detailsUrl = weatherDetailsBaseUrl + '?location=' + encodeURIComponent(locationName);
