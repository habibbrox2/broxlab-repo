    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">All Mobiles</h1>
                <p class="text-sm text-white/60 mt-0.5 max-w-md">Manage mobile devices in your catalog.</p>
            </div>
        </div>
        <div class="flex gap-2">
            <a href="/admin/mobiles/create" class="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                <i class="lucide lucide-plus w-4 h-4"></i> Add New Mobile
            </a>
        </div>
    </div>
</div>
...
<section class="mb-6 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
        ...
        <div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">Filter Mobiles</h3>
            <p class="text-xs text-slate-400 dark:text-slate-600">Search by brand/model, sort, filter by status, and adjust page size</p>
        </div>
    </div>
    <form class="p-5" method="get" action="">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-6">
            <div class="space-y-1.5 xl:col-span-2">
                <label for="mobile-search" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Search</label>
                <div class="relative">
                    <i class="lucide lucide-search absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-600"></i>
                    <input id="mobile-search" type="text" name="search" value="{{ $pagination['search'] }}" placeholder="Search brand or model..."
            ...
            <div class="space-y-1.5">
                <label for="mobile-sort" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Sort By</label>
                <select id="mobile-sort" name="sort"
                    ...
                    <option value="brand_name" {{ $pagination['sort'] === 'brand_name' ? 'selected' : '' }}>Brand Name</option>
                    <option value="model_name" {{ $pagination['sort'] === 'model_name' ? 'selected' : '' }}>Model Name</option>
                    <option value="official_price" {{ $pagination['sort'] === 'official_price' ? 'selected' : '' }}>Official Price</option>
                    <option value="status" {{ $pagination['sort'] === 'status' ? 'selected' : '' }}>Status</option>
                    <option value="created_at" {{ $pagination['sort'] === 'created_at' ? 'selected' : '' }}>Created At</option>
            ...
            <div class="space-y-1.5">
                ...
                <select id="mobile-order" name="order"
                    class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/60 px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 cursor-pointer focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/10">
                    <option value="ASC" {{ $pagination['order'] === 'ASC' ? 'selected' : '' }}>A-Z / Low-High</option>
                    <option value="DESC" {{ $pagination['order'] === 'DESC' ? 'selected' : '' }}>Z-A / High-Low</option>
                </select>
            </div>
            <div class="space-y-1.5">
            ...
            <div class="space-y-1.5">
                <label for="mobile-limit" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Per Page</label>
...
<section class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
        ...
        <div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">All Mobiles</h3>
            <p class="text-xs text-slate-400 dark:text-slate-600">Brand, model, pricing, status, and management actions</p>
        </div>
    </div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="border-b border-slate-100 dark:border-slate-800">
                    ...
                    <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400 dark:text-slate-600 whitespace-nowrap">Brand</th>
                    <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400 dark:text-slate-600 whitespace-nowrap">Model</th>
                ...
                <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors duration-100">
                    ...
                    <td class="px-4 py-3">
                        <div class="text-sm">
                            ...
                            @else
                                <span class="text-slate-400 dark:text-slate-600">Not set</span>
                            @endif
                        </div>
                    </td>
...
<section class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">No mobiles found</h3>
        <p class="text-sm text-slate-400 dark:text-slate-600 mb-5 max-w-xs">Start by adding your first mobile device to the catalog!</p>
        <a href="/admin/mobiles/create" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:-translate-y-0.5 transition-all duration-150">
            <i class="lucide lucide-plus w-4 h-4"></i> Add First Mobile
        </a>
    </div>
</section>
@endif
