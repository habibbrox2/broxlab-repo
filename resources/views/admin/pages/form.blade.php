...
{{-- ── Main Form ── --}}
<form method="page"
      action="{{ $isCreate ? '/admin/pages/create' : '/admin/pages/edit/'.($item->id ?? 0) }}"
      data-autosave="true"
      class="space-y-6">
    @csrf
    ...
    @unless ($isCreate)
        ...
        <input type="hidden" id="page_id" value="{{ $item->id ?? 0 }}">
    @endunless
                    ...
                    <div class="p-5 space-y-3">
                        ...
                        <div class="space-y-2.5">
                            <button type="submit"
                                ...
                                {{ $isCreate ? 'Publish Page' : 'Update Page' }}
                            </button>
                            <a href="/admin/pages"
...
(function () {
    ...
    // Slug preview + availability check (legacy /api/pages/check_permalink)
    ...
    function checkSlug() {
        ...
        var url = '/api/pages/check_permalink?slug=' + encodeURIComponent(slug) + (pageId ? '&exclude_id=' + pageId : '');
