...
@section('title', 'Create Permission — '.($appSettings['site_name'] ?? 'BroxLab'))
    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            <div class="w-11 h-11 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center flex-shrink-0">
                <i class="lucide lucide-lock-keyhole w-5 h-5 text-white"></i>
            </div>
            <div>
                <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60 mb-1">RBAC — Role-Based Access Control</p>
                <h1 class="text-xl font-bold text-white">{{ t('Create Permission') }}</h1>
                <p class="text-sm text-white/60 mt-0.5">{{ t('Define a new permission to control access') }}</p>
            </div>
        </div>
        ...
        <div class="px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
            <div class="flex items-center gap-3">
                <div class="w-7 h-7 rounded-lg bg-cyan-50 dark:bg-cyan-900/30 flex items-center justify-center flex-shrink-0">
                    <i class="lucide lucide-form-input w-4 h-4 text-cyan-600 dark:text-cyan-400"></i>
                </div>
                <div>
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ t('Permission Details') }}</h3>
                    <p class="text-xs text-slate-400 dark:text-slate-600">{{ t('Enter the permission information') }}</p>
                </div>
            </div>
        </div>
        <div class="p-5 sm:p-6 space-y-5">
            <form method="post" action="/admin/permissions/create" class="space-y-5">
                @csrf

                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1.5">
                        {{ t('Permission Name') }} <span class="text-rose-500 ml-0.5" aria-hidden="true">*</span>
                    </label>
                    <input type="text" name="name" placeholder="{{ t('e.g., users.view, posts.edit, admin.delete') }}" required maxlength="100"
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-600 mt-1">{{ t('Dot notation recommended (e.g., module.action)') }}</p>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-600 mt-1">{{ t('The module this permission belongs to') }}</p>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" rows="2" placeholder="{{ t('What does this permission allow?') }}" maxlength="500"
                        class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/60 px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/10"></textarea>
                    <p class="text-xs text-slate-400 dark:text-slate-600 mt-1">{{ t('A brief description of what this permission grants') }}</p>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-600 hover:bg-cyan-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-cyan-500/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                        <i class="lucide lucide-check-circle w-4 h-4"></i> {{ t('Create Permission') }}
                    </button>
