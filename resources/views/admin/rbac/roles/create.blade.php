    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">{{ t('Create Role') }}</h1>
                <p class="text-sm text-white/60 mt-0.5">{{ t('Define a new role with optional permissions') }}</p>
            </div>
        </div>
        ...
        <div class="px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-600">{{ t('Enter the role information') }}</p>
                </div>
            </div>
        </div>
        <div class="p-5 sm:p-6 space-y-5">
            <form method="post" action="/admin/roles/create" class="space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" placeholder="{{ t('e.g., Editor, Moderator, Subscriber') }}" required maxlength="50"
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-600 mt-1">{{ t('Unique name for this role (max 50 characters)') }}</p>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" rows="2" placeholder="{{ t('What is this role for?') }}" maxlength="500"
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-600 mt-1">{{ t('A brief description of the role\'s purpose') }}</p>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-emerald-500/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                        <i class="lucide lucide-check-circle w-4 h-4"></i> {{ t('Create Role') }}
                    </button>
