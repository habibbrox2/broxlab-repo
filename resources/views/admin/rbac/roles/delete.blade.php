    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">Delete Role</h1>
                <p class="text-sm text-white/60 mt-0.5">This action is irreversible. The role and its permissions will be permanently deleted.</p>
            </div>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-rose-200 dark:border-rose-800 bg-white dark:bg-slate-900 shadow-sm">
        ...
        <div class="p-5 sm:p-6 space-y-5">
            ...
            <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                <h4 class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">Role Details</h4>
                <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Super Admin:</span>
            ...
            @if(count($role['users']) > 0)
                <div class="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800">
                    <p class="text-sm text-amber-800 dark:text-amber-400 flex items-start gap-2">
                        ...
                        <span><strong>Warning:</strong> This role is currently assigned to <strong>{{ count($role['users']) }} user(s)</strong>. Deleting this role will remove the role assignment from these users.</span>
                    </p>
                </div>
            @endif
            ...
            @if(count($role['permissions']) > 0)
                <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700">
                    <p class="text-sm text-slate-600 dark:text-slate-400 mb-2">Permissions that will be disassociated:</p>
