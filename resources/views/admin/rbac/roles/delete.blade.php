...
@section('title', 'Delete Role — '.($appSettings['site_name'] ?? 'BroxLab'))
    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">{{ t('Delete Role') }}</h1>
                <p class="text-sm text-white/60 mt-0.5">{{ t('This action is irreversible. The role and its permissions will be permanently deleted.') }}</p>
            </div>
        </div>
        ...
        <div class="p-5 sm:p-6 space-y-5">
            <div class="rounded-xl border border-rose-100 dark:border-rose-800 bg-rose-50/40 dark:bg-rose-900/10 px-4 py-3">
                <p class="text-sm text-slate-700 dark:text-slate-300">
                    {{ t('Are you sure you want to permanently delete the role') }}
                    <span class="font-semibold text-rose-600 dark:text-rose-400">{{ $role['name'] }}</span>?
                    This action cannot be undone.
                </p>
            </div>

            <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                <h4 class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{{ t('Role Details') }}</h4>
                <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Ranking:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $role['ranking'] }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">{{ t('Super Admin:') }}</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $role['is_super_admin'] ? 'Yes' : 'No' }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Description:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $role['description'] ?: '—' }}</span>
                    </div>
                </div>
            </div>
            ...
            @if(count($role['users']) > 0)
                <div class="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800">
                    <p class="text-sm text-amber-800 dark:text-amber-400 flex items-start gap-2">
                        ...
                        <span><strong>Warning:</strong> {{ t('This role is currently assigned to') }} <strong>{{ count($role['users']) }} user(s)</strong>. Deleting this role will remove the role assignment from these users.</span>
                    </p>
                </div>
            @endif
            ...
            @if(count($role['permissions']) > 0)
                <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700">
                    <p class="text-sm text-slate-600 dark:text-slate-400 mb-2">{{ t('Permissions that will be disassociated:') }}</p>
            ...
            <div class="flex flex-wrap gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                <form action="/admin/roles/delete/{{ $role['id'] }}" method="post" onsubmit="return confirm('Are you absolutely sure? This action cannot be undone.');">
                    @csrf
                ...
                <a href="/admin/roles" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
