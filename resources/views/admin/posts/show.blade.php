...
@section('title', ($post->title ?? 'Post') . ' — '.($appSettings['site_name'] ?? 'BroxLab'))
    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">{{ $post->title ?? 'Post' }}</h1>
                <p class="text-sm text-white/60 mt-0.5">{{ t('View post details and content.') }}</p>
            </div>
        </div>
        <div class="flex gap-2">
            <a href="/admin/posts" class="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150"><i class="lucide lucide-arrow-left w-4 h-4"></i> Back</a>
            <a href="/admin/posts/edit/{{ $post->id ?? 0 }}" class="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150"><i class="lucide lucide-pencil w-4 h-4"></i> {{ t('Edit') }}</a>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1.25fr)_320px] gap-5">
    <div class="space-y-5">
        <div class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            ...
            <div class="p-5"><div class="prose prose-sm dark:prose-invert max-w-none text-slate-700 dark:text-slate-300">{!! $post->content ?? '' !!}</div></div>
        </div>
    </div>
    <div class="space-y-5">
        <div class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
                ...
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ t('Post Info') }}</h3>
            </div>
            <div class="p-5 space-y-4">
                <div><p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Author</p><p class="text-sm text-slate-700 dark:text-slate-300">{{ $post->author ?? 'N/A' }}</p></div>
                <div><p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Slug</p><p class="text-sm font-mono text-slate-700 dark:text-slate-300">{{ $post->slug ?? '' }}</p></div>
                ...
                <div><p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Created</p><p class="text-sm text-slate-700 dark:text-slate-300">{{ isset($post->created_at) ? \Illuminate\Support\Carbon::parse($post->created_at)->format('d M Y H:i') : 'N/A' }}</p></div>
                <div><p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">Updated</p><p class="text-sm text-slate-700 dark:text-slate-300">{{ isset($post->updated_at) ? \Illuminate\Support\Carbon::parse($post->updated_at)->format('d M Y H:i') : 'N/A' }}</p></div>
            </div>
        </div>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
                ...
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ t('Categories') }}</h3>
            </div>
