...
@section('title', 'Delete User — '.($appSettings['site_name'] ?? 'BroxLab'))
            ...
            <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        ...
                        <span class="font-medium text-slate-900 dark:text-white">{{ $user['full_name'] ?: '—' }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Status:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ ucfirst($user['status']) }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Created:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $user['created_at'] ? \Carbon\Carbon::parse($user['created_at'])->format('M j, Y') : '—' }}</span>
                    </div>
                </div>
            </div>
