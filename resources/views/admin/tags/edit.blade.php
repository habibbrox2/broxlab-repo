...
@section('title', 'Edit Tag — '.($appSettings['site_name'] ?? 'BroxLab'))
    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">{{ t('Edit Tag') }}</h1>
                <p class="text-sm text-white/60 mt-0.5 max-w-md">{{ t('Update tag details.') }}</p>
            </div>
        </div>
...
<div class="max-w-3xl">
    <div class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
            ...
            <div>
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white">Tag: {{ $tag->name ?? '' }}</h3>
                <p class="text-xs text-slate-400 dark:text-slate-600">{{ t('Edit the tag name and URL slug') }}</p>
            </div>
        </div>
        <div class="p-5 sm:p-6">
            <form method="post" action="/admin/tags/edit/{{ $tag->id ?? 0 }}" class="space-y-5">
                @csrf
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" value="{{ $tag->name ?? '' }}" placeholder="{{ t('Enter tag name') }}" required
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-500/10 bg-white dark:bg-slate-800/60 transition-all duration-150">
                        ...
                        <input type="text" name="slug" value="{{ $tag->slug ?? '' }}" placeholder="tag-url-slug" required
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-500/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                        <i class="lucide lucide-check-circle w-4 h-4"></i> {{ t('Update Tag') }}
                    </button>
