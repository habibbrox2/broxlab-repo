    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-white">{{ t('Manage Tags') }}</h1>
                <p class="text-sm text-white/60 mt-0.5 max-w-md">{{ t('Create and organize tags to label your content.') }}</p>
            </div>
        </div>
        <div class="flex gap-2">
            <a href="/admin/tags/create" class="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                <i class="lucide lucide-plus w-4 h-4"></i> {{ t('Add New Tag') }}
            </a>
        </div>
    </div>
</div>
...
<section class="mb-6 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
        ...
        <div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ t('Filter Tags') }}</h3>
    ...
    <form class="p-5" method="get" action="">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <div class="space-y-1.5">
                <label for="tag-search" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">{{ t('Search') }}</label>
                <div class="relative">
                    <i class="lucide lucide-search absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-600"></i>
                    <input id="tag-search" type="text" name="search" value="{{ $pagination['search'] }}" placeholder="{{ t('Search by name...') }}"
            ...
            <div class="space-y-1.5">
                <label for="tag-sort" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">{{ t('Sort By') }}</label>
            ...
            <div class="space-y-1.5">
                <label for="tag-limit" class="block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400 mb-1">{{ t('Per Page') }}</label>
...
<section class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
        ...
        <div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ t('All Tags') }}</h3>
...
<section class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
    <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ t('No tags found') }}</h3>
        <p class="text-sm text-slate-400 dark:text-slate-600 mb-5 max-w-xs">{{ t('Start by creating your first tag!') }}</p>
        <a href="/admin/tags/create" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:-translate-y-0.5 transition-all duration-150">
            <i class="lucide lucide-plus w-4 h-4"></i> {{ t('Create First Tag') }}
        </a>
    </div>
</section>
@endif
