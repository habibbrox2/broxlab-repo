...
@section('title', 'Delete Service — '.($appSettings['site_name'] ?? 'BroxLab'))
            ...
            <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        ...
                        <span class="font-medium text-slate-900 dark:text-white">{{$service['title'] ?? $service->title ?? $service['name'] ?? $service->name ?? 'N/A'}}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Slug:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{$service['slug'] ?? $service->slug ?? 'N/A'}}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Status:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{$service['status'] ?? $service->status ?? 'N/A'}}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Created:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{($service['created_at'] ?? $service->created_at) ? \Carbon\Carbon::parse($service['created_at'] ?? $service->created_at)->format('M j, Y g:i A') : 'N/A'}}</span>
                    </div>
                </div>
            </div>
