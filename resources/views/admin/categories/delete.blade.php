...
@section('title', 'Delete Category — '.($appSettings['site_name'] ?? 'BroxLab'))
...
<div class="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-red-900 to-rose-900 text-white shadow-xl shadow-slate-900/20">
    ...
    <div class="relative px-6 py-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60 mb-1">{{ t('Categories') }}</p>
                <h1 class="text-xl font-bold text-white">{{ t('Delete Category') }}</h1>
        ...
        <div class="p-5 sm:p-6 space-y-5">
            <div class="rounded-xl border border-rose-100 dark:border-rose-800 bg-rose-50/40 dark:bg-rose-900/10 px-4 py-3">
                <p class="text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <span class="font-semibold text-rose-600 dark:text-rose-400">{{ $category->name ?? 'Untitled' }}</span>?
                </p>
            </div>

            <div class="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                <h4 class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{{ t('Category Details') }}</h4>
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Name:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $category->name ?? 'N/A' }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">Slug:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $category->slug ?? 'N/A' }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 dark:text-slate-500">ID:</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $category->id ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                <form action="/admin/categories/delete/{{ $category->id }}" method="post">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-rose-600 hover:bg-rose-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-rose-500/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
                        <i class="lucide lucide-trash-2 w-4 h-4"></i> {{ t('Delete Category') }}
                    </button>
                </form>
                <a href="/admin/categories" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-150">
