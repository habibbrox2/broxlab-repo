    ...
    <section class="relative w-full max-w-[480px] overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_24px_80px_-30px_rgba(15,23,42,0.3)]">
        ...
        <div class="px-6 pb-2 pt-8 sm:px-8 sm:pt-10">
            ...
            <div class="text-center">
                <h1 class="text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">{{ t('Verify your email') }}</h1>
        ...
        <div class="px-6 pb-8 sm:px-8 sm:pb-10">
            ...
            <form method="POST" action="{{ route('verify-email.submit') }}" class="space-y-5" novalidate>
                ...
                <div class="space-y-1.5">
                    <label for="verification_token" class="block text-sm font-semibold text-slate-700">{{ t('Verification token') }}</label>
                ...
                <button type="submit"
                    ...
                    <i class="lucide lucide-check-circle h-4 w-4"></i> {{ t('Verify email') }}
                </button>
            </form>

            <div class="mt-6 border-t border-slate-100 pt-5 text-center">
                <p class="text-sm text-slate-500">{{ t('Didn\'t get the email?') }}</p>
