...
export class Agent {
    private static sessionApproved = false;
        ...
        if (needsApproval && approvalMode !== 'Auto Approve') {
            if (approvalMode === 'Ask Once Per Session' && Agent.sessionApproved) {
                ...
                while (!userApproved) {
                    ...
                    if (selection === 'Allow') {
                        userApproved = true;
                        if (approvalMode === 'Ask Once Per Session') Agent.sessionApproved = true;
    ...
    public async handleRequest(prompt: string, history: any[], onUpdate: (update: any) => void) {
        if (!this.workspaceRoot) {
