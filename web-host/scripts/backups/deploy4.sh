...
if ! command -v node &> /dev/null; then
    ...
    echo "Fix:"
    echo "  1) Install Node.js 20 via NVM: nvm install 20"
    echo "  2) Activate: nvm use 20"
    echo "  3) Re-run deploy script"
    exit 1
fi
...
if ! command -v npm &> /dev/null; then
    ...
    echo "Fix:"
    echo "  1) Ensure Node.js 20 is installed via NVM"
    echo "  2) Re-run deploy script"
    exit 1
fi
...
if [ "$NODE_MAJOR" != "20" ]; then
    echo "⚠️  WARNING: Node.js major version should be 20 (found: ${NODE_MAJOR:-unknown})."
    echo "Fix:"
    echo "  1) nvm install 20"
    echo "  2) nvm use 20"
    echo "  3) Re-run deploy script"
fi

if ! command -v pm2 &> /dev/null; then
    echo "ℹ️  PM2 not found (will install later if needed)."
fi

echo "ℹ️  Puppeteer is optional on cPanel; HTTP-only mode supported via SCRAPER_DISABLE_BROWSER=true."
...
if command -v pm2 &> /dev/null; then
    ...
    pm2 delete scraper-direct-api 2>/dev/null || true
    ...
    # Ensure Redis is running before starting scraper services
    if ! redis-cli ping &>/dev/null; then
        ...
        if redis-cli ping &>/dev/null; then
            ...
            echo "⚠️ WARNING: Redis failed to start, scraper-api may fail"
        fi
    else
    ...
    pm2 start $NEW_RELEASE/src/ecosystem.config.cjs 2>/dev/null || {
        ...
        pm2 start $NEW_RELEASE/src/api/direct-scraper.js --name scraper-direct-api 2>/dev/null || true
        ...
        # Start new Bangla scraper if available
        if [[ -f "$NEW_RELEASE/src/bdnews-scraper/scraper.js" ]]; then
            pm2 start $NEW_RELEASE/src/bdnews-scraper/scraper.js --name bangla-scraper -- --sources=all --max=12 2>/dev/null || true
            echo "✅ Bangla scraper started"
        fi
    }
    pm2 save 2>/dev/null || true
    ...
    nohup node $NEW_RELEASE/src/api/direct-scraper.js > $SHARED/logs/scraper-direct-api.log 2>&1 &
